...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    训练帧集（train frames）：用于学习 DiMP 分类模型并获取 IoU-Net 的调制向量。
    测试帧集（test frames）：用于计算预测的 DiMP 模型的目标分类损失以及 IoU-Net 的 IoU 预测损失。
    ...
    1.随机选择数据集。
    2.从选定的数据集中随机选择一个序列。
    3.从序列中随机选择一个基准帧（base frame）。
    4.从基准帧的前后一定范围内采样训练帧和测试帧，范围由 max_gap 决定。如果在初始范围内找不到足够可见的帧，则逐渐增加 max_gap，直到找到足够的帧。
    ...
    def __getitem__(self, index):
        ...
        if is_video_dataset:
            ...
            if self.frame_sample_mode == 'interval':
                # Sample frame numbers within interval defined by the first frame 如果是 'interval' 模式，训练和测试帧在基准帧的前后 max_gap 范围内随机采样
                ...
                # Sample test and train frames in a causal manner, i.e. test_frame_ids > train_frame_ids 如果是 'causal' 模式，测试帧必须在训练帧之后
                while test_frame_ids is None:
            ...
            # In case of image dataset, just repeat the image to generate synthetic video 处理非视频数据集：对于图像数据集，重复使用同一帧以生成合成视频。
            train_frame_ids = [1] * self.num_train_frames
            test_frame_ids = [1] * self.num_test_frames

        # 获取帧数据和注释：调用 dataset.get_frames 方法获取训练和测试帧及其注释
