...
class ResNet(Backbone):
    ...
    def forward(self, x, output_layers=None):
        ...
        outputs = OrderedDict() #可变对象的引用： 当你传递一个可变对象（如列表或字典）给函数时，函数内部对该对象的修改会反映到外部，因为它们引用的是同一个对象;不可变对象的引用;当你传递一个不可变对象（如整数或字符串）给函数时，函数内部对该对象的“修改”实际上是创建了一个新的对象，并将其绑定到局部变量，原始对象保持不变。
