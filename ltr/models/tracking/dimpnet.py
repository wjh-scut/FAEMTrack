...
class DiMPnet(nn.Module):
    ...
    def __init__(self, feature_extractor, classifier, bb_regressor, classification_layer, bb_regressor_layer):
        ...
        self.output_layers = sorted(list(set(self.classification_layer + self.bb_regressor_layer))) #合并并排序分类层和回归层，确保特征提取时所需的所有层都被提取
    ...
    def forward(self, train_imgs, test_imgs, train_bb, test_proposals, *args, **kwargs):
        ...
        args:
            ...
            *args, **kwargs:  These are passed to the classifier module.额外的参数，传递给分类器模块
        returns:
            test_scores:  Classification scores on the test samples.
            iou_pred:  Predicted IoU scores for the test_proposals."""

        assert train_imgs.dim() == 5 and test_imgs.dim() == 5, 'Expect 5 dimensional inputs'  # (3,20,3,352,352)

        # Extract backbone features
        train_feat = self.extract_backbone_features(train_imgs.reshape(-1, *train_imgs.shape[-3:]))  # layer2 (60,512,44,44),layer3 (60,1024,22,22)
        test_feat = self.extract_backbone_features(test_imgs.reshape(-1, *test_imgs.shape[-3:]))  # layer2 (60,512,44,44),layer3 (60,1024,22,22)
        ...
        train_feat_iou = self.get_backbone_bbreg_feat(train_feat) # list:2 (60,512,44,44),(60,1024,22,22)
        test_feat_iou = self.get_backbone_bbreg_feat(test_feat)  # list:2 (60,512,44,44),(60,1024,22,22)

        # Run the IoUNet module
        iou_pred = self.bb_regressor(train_feat_iou, test_feat_iou, train_bb, test_proposals)  # (3,18,128)
...
@model_constructor #装饰器 https://blog.csdn.net/m0_38123128/article/details/140260446
...
def dimpnet50(filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
    ...
    clf_feature_extractor = clf_features.FAEMTrack(norm_scale=norm_scale)
