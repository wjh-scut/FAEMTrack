...
# from tools.plotting import show_tensor
...
class BiLevelRoutingAttention_nchw(nn.Module):
    ...
    * different pooling behavior (count_include_pad=False)
    ...
    def forward(self, x:Tensor, ret_attn_mask=False):
        ...
        # region_size = (1, 1)  # 为了适配mobilevit，xyl20231030
        ...
        _, idx_r = torch.topk(a_r, k=self.topk, dim=-1) # n(hw)k long tensor, 计算：routing region indices matrix (n, h*w, topk)
        ...
        return output
        # return output, idx_r.sum(dim=1).squeeze(0).reshape(-1,h,w)


if __name__ == "__main__":
    in_tensor = torch.ones((1, 256, 31, 31))

    cb = BiLevelRoutingAttention_nchw(dim=256, num_heads=1)
