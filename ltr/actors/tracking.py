...
class KLDiMPActor(BaseActor):
    """Actor for training the DiMP network."""
    def __init__(self, net, objective, loss_weight=None):  # objective: 损失函数或一组损失函数，用于计算训练过程中的误差
        super().__init__(net, objective)
        if loss_weight is None:
            loss_weight = {'bb_ce': 1.0}
        self.loss_weight = loss_weight # loss_weight: 损失函数的权重，用于调整不同损失在总损失中的贡献。如果未提供，默认为 {'bb_ce': 1.0}

    def __call__(self, data): # 当调用该类的实例时，就会执行__call__方法，这允许类的实例像函数一样被调用
        """
        args:
            data - The input data, should contain the fields 'train_images', 'test_images', 'train_anno',
                    'test_proposals', 'proposal_iou' and 'test_label'.
