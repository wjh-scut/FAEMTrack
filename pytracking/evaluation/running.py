...
def _save_tracker_output(seq: Sequence, tracker: Tracker, output: dict):
    ...
    def save_bb(file, data):
        ...
        np.savetxt(file, tracked_bb, delimiter='\t', fmt='%d')  # pytracking保存的分割符号是 '\t'，pysot的分割符号是',' （但是不影响，我已经在pysot-toolkit-master中兼容了三种分割符号：有:'\t', ',', ' '）
...
def run_sequence(seq: Sequence, seq2: Sequence, tracker: Tracker, debug=False, visdom_info=None):
    ...
    if _results_exist() and not debug:
        print('{} {} {} FPS: {}'.format(tracker.name, tracker.parameter_name, tracker.run_id, -1))
        return
    ...
    if debug:
        ...
        output, output2 = tracker.run_sequence(seq, seq2, debug=debug, visdom_info=visdom_info)
    else:
        try:
            # output = tracker.run_sequence(seq, debug=debug, visdom_info=visdom_info)
            output, output2 = tracker.run_sequence(seq, seq2, debug=debug, visdom_info=visdom_info)
...
def run_sequence2(seq: Sequence, tracker: Tracker, debug=False, visdom_info=None):
    """Runs a tracker on a sequence."""
    ...
    if debug:
        output = tracker.run_sequence2(seq, debug=debug, visdom_info=visdom_info)  # multi-pytracking xyl20231209s
    else:
        try:
            output = tracker.run_sequence2(seq, debug=debug, visdom_info=visdom_info)  # multi-pytracking xyl20231209s
        except Exception as e:
...
def run_dataset(dataset, trackers, debug=False, threads=0, visdom_info=None):
    ...
    if mode == 'sequential':
        ...
        #         run_sequence2(seq, tracker_info, debug=debug, visdom_info=visdom_info)
        # for seq in dataset:  # 逐个加载数据集的序列 xyl20231209
        for v_idx in range(0, len(dataset), 2):  # multi-pytracking xyl20231209s
            seq = dataset[v_idx]
            seq2 = dataset[v_idx + 1]
            for tracker_info in trackers:
                # run_sequence(seq, tracker_info, debug=debug, visdom_info=visdom_info)
                print('------------------- {} / {} --------------------:'.format(v_idx, len(dataset)))
                run_sequence(seq, seq2, tracker_info, debug=debug, visdom_info=visdom_info)  # multi-pytracking xyl20231209s
    elif mode == 'parallel':
