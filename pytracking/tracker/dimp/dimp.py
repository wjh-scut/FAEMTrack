...
class DiMP(BaseTracker):
    ...
    def initialize_features2(self):
        if not getattr(self, 'features_initialized', False):
            self.params.net.initialize()
        self.features_initialized2 = True
    ...
    def initialize2(self, image, info: dict) -> dict:
        # Initialize some stuff
        self.frame_num2 = 1
        if not self.params.has('device'):
            self.params.device = 'cuda' if self.params.use_gpu else 'cpu'
        ...
        # Get target position and size
        state = info['init_bbox']
        self.pos2 = torch.Tensor([state[1] + (state[3] - 1) / 2, state[0] + (state[2] - 1) / 2])
        self.target_sz2 = torch.Tensor([state[3], state[2]])

        # Get object id
        self.object_id2 = info.get('object_ids', [None])[0]
        self.id_str2 = '' if self.object_id2 is None else ' {}'.format(self.object_id2)

        # Set sizes
        self.image_sz2 = torch.Tensor([im.shape[2], im.shape[3]])
        sz = self.params.image_sample_size
        sz = torch.Tensor([sz, sz] if isinstance(sz, int) else sz)
        if self.params.get('use_image_aspect_ratio', False):
            sz = self.image_sz2 * sz.prod().sqrt() / self.image_sz2.prod().sqrt()
            stride = self.params.get('feature_stride', 32)
            sz = torch.round(sz / stride) * stride
        self.img_sample_sz2 = sz
        self.img_support_sz2 = self.img_sample_sz2

        # Set search area
        search_area = torch.prod(self.target_sz2 * self.params.search_area_scale).item()
        self.target_scale2 = math.sqrt(search_area) / self.img_sample_sz2.prod().sqrt()

        # Target size in base scale
        self.base_target_sz2 = self.target_sz2 / self.target_scale2
        ...
        # Setup scale bounds
        self.min_scale_factor2 = torch.max(10 / self.base_target_sz2)
        self.max_scale_factor2 = torch.min(self.image_sz2 / self.base_target_sz2)

        # Extract and transform sample
        init_backbone_feat = self.generate_init_samples2(im)

        # Initialize classifier
        self.init_classifier2(init_backbone_feat)

        # Initialize IoUNet
        if self.params.get('use_iou_net', True):
            self.init_iou_net2(init_backbone_feat)

        out = {'time': time.time() - tic2}
        return out

    def entropy_confidence(self, response_tensor):
        response_map = response_tensor.squeeze()
        flattened_map = response_map.flatten().cpu().numpy()
        # 添加一个小常数以避免除零或熵计算的问题
        prob_distribution = flattened_map / (np.sum(flattened_map) + 1e-10)
        # 避免log(0)的产生
        prob_distribution = np.clip(prob_distribution, 1e-10, 1)
        map_entropy = entropy(prob_distribution)
        # 确保返回的熵值不是零，从而不导致全零的影响
        return 1 / (1 + map_entropy)

    def track(self, image, info: dict = None) -> dict:
        if info and 'gt_bbox' in info:
            self.gt_bbox_1 = info['gt_bbox']
        ...
        test_x = self.get_classification_features(backbone_feat)  # (1,512,22,22)

        # Location of sample
        sample_pos, sample_scales = self.get_sample_location(sample_coords) #sample_pos指的是搜索区域图像的中心点位置坐标（在原始图像im的坐标系下），sample_scales代表搜索区域图像与采样图像（im_patches）之间的比例大小

        # Compute classification scores
        scores_raw = self.classify_target(test_x)  # 利用滤波器和搜索特征进行卷积得到分类分数 (1,1,23,23)
        #test_x_ = self.net.mvt(test_x)
        scores_raw2 = self.classify_target2(test_x)  # 利用滤波器2和搜索特征进行卷积得到分类分数

        # ----------------------------------------------------------------------------------------------加权融合
        max_scores = torch.max(scores_raw.squeeze()).item()
        max_scores2 = torch.max(scores_raw2.squeeze()).item()

        if max_scores >= 0.8:
            pass
        elif max_scores2 > 0.6 > max_scores and max_scores2 - max_scores >= 0.2:
            scores_raw = scores_raw2
        elif max_scores2 > 0.6:
            response_tensors = [scores_raw, scores_raw2]
            confidences = [self.entropy_confidence(rm) for rm in response_tensors]
            confidence_sum = sum(confidences) + 1e-10
            weights = [c / confidence_sum for c in confidences]
            scores_raw = sum(w * rm.squeeze() for w, rm in zip(weights, response_tensors)).unsqueeze(0).unsqueeze(
                0)
        else:
            pass
        ...
        # Localize the target
        if not self.multi_tracking:
            translation_vec, scale_ind, s, flag = self.localize_target(scores_raw, sample_pos, sample_scales)
            new_pos = sample_pos[scale_ind, :] + translation_vec
            counting = 1
        else:
            translation_vec1, scale_ind1, s1, flag1 = self.localize_target(scores_raw, sample_pos, sample_scales)
            translation_vec2, scale_ind2, s2, flag2 = self.localize_target2(scores_raw2, sample_pos, sample_scales)

            score_map1 = s1[scale_ind1, ...]
            max_score1 = torch.max(score_map1).item()
            score_map2 = s2[scale_ind2, ...]
            max_score2 = torch.max(score_map2).item()
            if max_score1 < self.params.theta < max_score2:
                translation_vec, scale_ind, s, flag = translation_vec2, scale_ind2, s2, flag2
                #print('drone2 work in drone1 ')
                counting = 2
            else:
                translation_vec, scale_ind, s, flag = translation_vec1, scale_ind1, s1, flag1
                counting = 1
            new_pos = sample_pos[scale_ind, :] + translation_vec
        # ----------------------------------------------------------------------------------------------multi-pytracking
        #print('drone1,{},{},{},{}'.format(self.frame_num, flag, torch.max(scores_raw.squeeze()).item(), torch.max(scores_raw2.squeeze()).item()))
    ...
    def track2(self, image, info: dict = None) -> dict:
        if info and 'gt_bbox' in info:
            self.gt_bbox_2 = info['gt_bbox']

        self.debug_info2 = {}

        self.frame_num2 += 1
        self.debug_info2['frame_num'] = self.frame_num2
        ...
        # Extract backbone features
        backbone_feat, sample_coords, im_patches = self.extract_backbone_features(im, self.get_centered_sample_pos2(),
        ...
        # Location of sample
        sample_pos, sample_scales = self.get_sample_location2(sample_coords)

        # Compute classification scores
        scores_raw2 = self.classify_target2(test_x)  # 利用滤波器2和搜索特征进行卷积得到分类分数
        #test_x_ = self.net.mvt(test_x)
        scores_raw = self.classify_target(test_x)  # 利用滤波器2和搜索特征进行卷积得到分类分数
        ...
        if max_scores2 >= 0.8:
            pass
        elif max_scores > 0.6 > max_scores2 and max_scores - max_scores2 >= 0.2:
            scores_raw2 = scores_raw
        elif max_scores > 0.6:
            response_tensors = [scores_raw, scores_raw2]
            confidences = [self.entropy_confidence(rm) for rm in response_tensors]
            confidence_sum = sum(confidences) + 1e-10
            weights = [c / confidence_sum for c in confidences]
            scores_raw2 = sum(w * rm.squeeze() for w, rm in zip(weights, response_tensors)).unsqueeze(0).unsqueeze(
                0)
        else:
            pass
        # ----------------------------------------------------------------------------------------------加权融合
        ...
        # Localize the target
        if not self.multi_tracking:
            translation_vec, scale_ind, s, flag = self.localize_target2(scores_raw2, sample_pos, sample_scales)
            new_pos = sample_pos[scale_ind, :] + translation_vec
            counting2 = 2
        else:
            translation_vec1, scale_ind1, s1, flag1 = self.localize_target(scores_raw, sample_pos, sample_scales)
            ...
            new_pos = sample_pos[scale_ind, :] + translation_vec
        # ----------------------------------------------------------------------------------------------multi-pytracking
        #print('drone2,{},{},{},{}'.format(self.frame_num2,flag,torch.max(scores_raw2.squeeze()).item(),torch.max(scores_raw.squeeze()).item()))
        # Update position and scale
        if flag != 'not_found':
            if self.params.get('use_iou_net', True):
                update_scale_flag = self.params.get('update_scale_when_uncertain', True) or flag != 'uncertain'
                if self.params.get('use_classifier', True):
                    self.update_state2(new_pos)
                self.refine_target_box2(backbone_feat, sample_pos[scale_ind, :], sample_scales[scale_ind], scale_ind,
                                        update_scale_flag)  # 得到位置和边界框
            elif self.params.get('use_classifier', True):
                self.update_state2(new_pos, sample_scales[scale_ind])
        ...
        if update_flag and self.params.get('update_classifier', False):
            ...
            # Create target_box and label for spatial sample
            target_box = self.get_iounet_box2(self.pos2, self.target_sz2, sample_pos[scale_ind, :],
                                              sample_scales[scale_ind])

            # Update the classifier model
            self.update_classifier2(train_x, target_box, learning_rate, s[scale_ind, ...])

        # Set the pos of the tracker to iounet pos
        if self.params.get('use_iou_net', True) and flag != 'not_found' and hasattr(self, 'pos_iounet'):
            self.pos2 = self.pos_iounet2.clone()

        score_map = s[scale_ind, ...]
        max_score = torch.max(score_map).item()

        # Visualize and set debug info
        self.search_area_box2 = torch.cat(
            (sample_coords[scale_ind, [1, 0]], sample_coords[scale_ind, [3, 2]] - sample_coords[scale_ind, [1, 0]] - 1))
        self.debug_info2['flag' + self.id_str2] = flag
        self.debug_info2['max_score' + self.id_str2] = max_score
        if self.visdom is not None:
            self.visdom.register(score_map, 'heatmap', 2, 'Score Map' + self.id_str2)
            self.visdom.register(self.debug_info2, 'info_dict', 1, 'Status')
        elif self.params.debug >= 2:
            show_tensor(score_map, 5, title='Max score = {:.2f}'.format(max_score))

        # Compute output bounding box
        new_state = torch.cat((self.pos2[[1, 0]] - (self.target_sz2[[1, 0]] - 1) / 2, self.target_sz2[[1, 0]]))
    ...
    def get_sample_location2(self, sample_coord):
        """Get the location of the extracted sample."""
        sample_coord = sample_coord.float()
        sample_pos = 0.5 * (sample_coord[:, :2] + sample_coord[:, 2:] - 1)
        sample_scales = ((sample_coord[:, 2:] - sample_coord[:, :2]) / self.img_sample_sz2).prod(dim=1).sqrt()
        return sample_pos, sample_scales
    ...
    def get_centered_sample_pos2(self):
        """Get the center position for the new sample. Make sure the target is correctly centered."""
        return self.pos2 + ((self.feature_sz2 + self.kernel_size2) % 2) * self.target_scale2 * \
               self.img_support_sz2 / (2 * self.feature_sz2)
    ...
    def classify_target2(self, sample_x: TensorList):
        """Classify target by applying the DiMP filter."""
        with torch.no_grad():
            scores = self.net.classifier.classify(self.target_filter2, sample_x)
        return scores
    ...
    def localize_target2(self, scores, sample_pos, sample_scales):
        """Run the target localization."""

        scores = scores.squeeze(1)

        preprocess_method = self.params.get('score_preprocess', 'none') #preprocess_method=none
        if preprocess_method == 'none':
            pass
        elif preprocess_method == 'exp':
            scores = scores.exp()
        ...
        score_filter_ksz = self.params.get('score_filter_ksz', 1)  #score_filter_ksz=1
        if score_filter_ksz > 1:
            assert score_filter_ksz % 2 == 1
            kernel = scores.new_ones(1, 1, score_filter_ksz, score_filter_ksz)
            scores = F.conv2d(scores.view(-1, 1, *scores.shape[-2:]), kernel, padding=score_filter_ksz // 2).view(
        ...
        if self.params.get('advanced_localization', False): #True
        ...
        # Compute translation vector and scale change factor
        output_sz = score_sz - (self.kernel_size2 + 1) % 2
        translation_vec = target_disp * (self.img_support_sz2 / output_sz) * sample_scales[scale_ind]

        return translation_vec, scale_ind, scores, None

    def localize_advanced(self, scores, sample_pos, sample_scales):
        """Run the target advanced localization (as in ATOM).它的目的是在给定的得分图和尺度样本中定位目标的位置，并根据不同的得分情况确定目标状态"""
        # 确定得分图的中心和输出尺寸
        sz = scores.shape[-2:]
        score_sz = torch.Tensor(list(sz)) #得分图的尺寸
        output_sz = score_sz - (self.kernel_size + 1) % 2 #去掉卷积核边缘后的尺寸
        score_center = (score_sz - 1) / 2 #得分图的中心点

        scores_hn = scores
        if self.output_window is not None and self.params.get('perform_hn_without_windowing', False): #如果定义了输出窗口，则应用该窗口对得分进行加权（这里未执行）
            scores_hn = scores.clone()
            scores *= self.output_window
        # 确定得分图的最大值（即目标位置的最佳估计）及其对应的尺度和位移
        max_score1, max_disp1 = dcf.max2d(scores) #使用 dcf.max2d 找到得分图的最大值位置，这是初步估计的目标位置
        ...
        # 通过最大得分与阈值的比较，判断目标是否未找到、存在不确定性或是困难样本。
        ...
        # Mask out target neighborhood 计算目标邻域的大小，并在得分图上屏蔽掉该邻域，以避免重复检测
        ...
        # Find new maximum 在屏蔽后的得分图上寻找新的最大值，以检测可能的干扰目标。
        ...
        # Handle the different cases 通过比较新的最大得分以及与初始位置的位移，判断目标状态，并返回相应的平移向量和状态。
    ...
    def localize_advanced2(self, scores, sample_pos, sample_scales):
        ...
        scores_hn = scores
        if self.output_window2 is not None and self.params.get('perform_hn_without_windowing', False):
            scores_hn = scores.clone()
            scores *= self.output_window2
        ...
        max_disp1 = max_disp1[scale_ind, ...].float().cpu().view(-1)
        target_disp1 = max_disp1 - score_center
        translation_vec1 = target_disp1 * (self.img_support_sz2 / output_sz) * sample_scale
        ...
        target_neigh_sz = self.params.target_neighborhood_scale * (self.target_sz2 / sample_scale) * (
                output_sz / self.img_support_sz2)
        ...
        max_disp2 = max_disp2.float().cpu().view(-1)
        target_disp2 = max_disp2 - score_center
        translation_vec2 = target_disp2 * (self.img_support_sz2 / output_sz) * sample_scale

        prev_target_vec = (self.pos2 - sample_pos[scale_ind, :]) / ((self.img_support_sz2 / output_sz) * sample_scale)
    ...
    def generate_init_samples2(self, im: torch.Tensor) -> TensorList:
        """Perform data augmentation to generate initial training samples."""
        ...
        if mode == 'inside':
            # Get new sample size if forced inside the image
            im_sz = torch.Tensor([im.shape[2], im.shape[3]])
            sample_sz = self.target_scale2 * self.img_sample_sz2
            shrink_factor = (sample_sz.float() / im_sz)
            if mode == 'inside':
                shrink_factor = shrink_factor.max()
            elif mode == 'inside_major':
            ...
            shrink_factor.clamp_(min=1, max=self.params.get('patch_max_scale_change', None))
            sample_sz = (sample_sz.float() / shrink_factor)
            self.init_sample_scale2 = (sample_sz / self.img_sample_sz2).prod().sqrt()
            tl = self.pos2 - (sample_sz - 1) / 2
            br = self.pos2 + sample_sz / 2 + 1
            global_shift = - ((-tl).clamp(0) - (br - im_sz).clamp(0)) / self.init_sample_scale2
        else:
            self.init_sample_scale2 = self.target_scale2
            global_shift = torch.zeros(2)

        self.init_sample_pos2 = self.pos2.round()

        # Compute augmentation size
        aug_expansion_factor = self.params.get('augmentation_expansion_factor', None)
        aug_expansion_sz = self.img_sample_sz2.clone()
        aug_output_sz = None
        if aug_expansion_factor is not None and aug_expansion_factor != 1:
            aug_expansion_sz = (self.img_sample_sz2 * aug_expansion_factor).long()
            aug_expansion_sz += (aug_expansion_sz - self.img_sample_sz2.long()) % 2
            aug_expansion_sz = aug_expansion_sz.float()
            aug_output_sz = self.img_sample_sz2.long().tolist()

        # Random shift for each sample
        get_rand_shift = lambda: None
        random_shift_factor = self.params.get('random_shift_factor', 0)
        if random_shift_factor > 0:
            get_rand_shift = lambda: ((torch.rand(
                2) - 0.5) * self.img_sample_sz2 * random_shift_factor + global_shift).long().tolist()

        # Always put identity transformation first, since it is the unaugmented sample that is always used
        self.transforms2 = [augmentation.Identity(aug_output_sz, global_shift.long().tolist())]

        augs = self.params.augmentation if self.params.get('use_augmentation', True) else {}

        # Add all augmentations
        if 'shift' in augs:
            self.transforms2.extend(
                [augmentation.Translation(shift, aug_output_sz, global_shift.long().tolist()) for shift in
                 augs['shift']])
        if 'relativeshift' in augs:
            get_absolute = lambda shift: (torch.Tensor(shift) * self.img_sample_sz2 / 2).long().tolist()
            self.transforms2.extend(
                [augmentation.Translation(get_absolute(shift), aug_output_sz, global_shift.long().tolist()) for shift in
                 augs['relativeshift']])
        if 'fliplr' in augs and augs['fliplr']:
            self.transforms2.append(augmentation.FlipHorizontal(aug_output_sz, get_rand_shift()))
        if 'blur' in augs:
            self.transforms2.extend(
                [augmentation.Blur(sigma, aug_output_sz, get_rand_shift()) for sigma in augs['blur']])
        if 'scale' in augs:
        ...
        # Extract augmented image patches
        im_patches = sample_patch_transformed(im, self.init_sample_pos2, self.init_sample_scale2, aug_expansion_sz,
                                              self.transforms2)
    ...
    def init_target_boxes2(self):
        """Get the target bounding boxes for the initial augmented samples."""
        self.classifier_target_box2 = self.get_iounet_box(self.pos2, self.target_sz2, self.init_sample_pos2,
                                                          self.init_sample_scale2)
        init_target_boxes = TensorList()
        for T in self.transforms2:
            init_target_boxes.append(self.classifier_target_box2 + torch.Tensor([T.shift[1], T.shift[0], 0, 0]))
        init_target_boxes = torch.cat(init_target_boxes.view(1, 4), 0).to(self.params.device)
        self.target_boxes2 = init_target_boxes.new_zeros(self.params.sample_memory_size, 4)
        self.target_boxes2[:init_target_boxes.shape[0], :] = init_target_boxes
        return init_target_boxes
    ...
    def init_memory2(self, train_x: TensorList):
        # Initialize first-frame spatial training samples
        self.num_init_samples2 = train_x.size(0)
        init_sample_weights = TensorList([x.new_ones(1) / x.shape[0] for x in train_x])

        # Sample counters and weights for spatial
        self.num_stored_samples2 = self.num_init_samples2.copy()
        self.previous_replace_ind2 = [None] * len(self.num_stored_samples2)
        self.sample_weights2 = TensorList([x.new_zeros(self.params.sample_memory_size) for x in train_x])
        for sw, init_sw, num in zip(self.sample_weights2, init_sample_weights, self.num_init_samples2):
            sw[:num] = init_sw

        # Initialize memory
        self.training_samples2 = TensorList(
            [x.new_zeros(self.params.sample_memory_size, x.shape[1], x.shape[2], x.shape[3]) for x in train_x])

        for ts, x in zip(self.training_samples2, train_x):
            ts[:x.shape[0], ...] = x
    ...
    def update_memory2(self, sample_x: TensorList, target_box, learning_rate=None):
        # Update weights and get replace ind
        replace_ind = self.update_sample_weights(self.sample_weights2, self.previous_replace_ind2,
                                                 self.num_stored_samples2, self.num_init_samples2, learning_rate)
        self.previous_replace_ind2 = replace_ind

        # Update sample and label memory
        for train_samp, x, ind in zip(self.training_samples2, sample_x, replace_ind):
            train_samp[ind:ind + 1, ...] = x

        # Update bb memory
        self.target_boxes2[replace_ind[0], :] = target_box

        self.num_stored_samples2 += 1
    ...
    def update_state2(self, new_pos, new_scale=None):
        # Update scale
        if new_scale is not None:
            self.target_scale2 = new_scale.clamp(self.min_scale_factor2, self.max_scale_factor2)
            self.target_sz2 = self.base_target_sz2 * self.target_scale2

        # Update pos
        inside_ratio = self.params.get('target_inside_ratio', 0.2)
        inside_offset = (inside_ratio - 0.5) * self.target_sz2
        self.pos2 = torch.max(torch.min(new_pos, self.image_sz2 - inside_offset), inside_offset)
    ...
    def get_iounet_box2(self, pos, sz, sample_pos, sample_scale):
        """All inputs in original image coordinates.
        Generates a box in the cropped image sample reference frame, in the format used by the IoUNet."""
        box_center = (pos - sample_pos) / sample_scale + (self.img_sample_sz2 - 1) / 2
        box_sz = sz / sample_scale
        target_ul = box_center - (box_sz - 1) / 2
        return torch.cat([target_ul.flip((0,)), box_sz.flip((0,))])
    ...
    def init_iou_net2(self, backbone_feat):
        # Setup IoU net and objective
        for p in self.net.bb_regressor.parameters():
            p.requires_grad = False

        # Get target boxes for the different augmentations
        self.classifier_target_box2 = self.get_iounet_box2(self.pos2, self.target_sz2, self.init_sample_pos2,
                                                           self.init_sample_scale2)
        target_boxes = TensorList()
        if self.params.iounet_augmentation:
            for T in self.transforms2:
                if not isinstance(T, (
                        ...
                        augmentation.Blur)):
                    break
                target_boxes.append(self.classifier_target_box2 + torch.Tensor([T.shift[1], T.shift[0], 0, 0]))
        else:
            target_boxes.append(self.classifier_target_box2 + torch.Tensor(
                [self.transforms2[0].shift[1], self.transforms2[0].shift[0], 0, 0]))
        target_boxes = torch.cat(target_boxes.view(1, 4), 0).to(self.params.device)
        ...
        # Get modulation vector
        self.iou_modulation2 = self.get_iou_modulation(iou_backbone_feat, target_boxes)
        if torch.is_tensor(self.iou_modulation2[0]):
            self.iou_modulation2 = TensorList([x.detach().mean(0) for x in self.iou_modulation2])
    ...
    def init_classifier2(self, init_backbone_feat):
        # Get classification features
        x = self.get_classification_features(init_backbone_feat)
        ...
        if 'dropout' in self.params.augmentation and self.params.get('use_augmentation', True):
            num, prob = self.params.augmentation['dropout']
            self.transforms2.extend(self.transforms2[:1] * num)
            x = torch.cat([x, F.dropout2d(x[0:1, ...].expand(num, -1, -1, -1), p=prob, training=True)])

        # Set feature size and other related sizes
        self.feature_sz2 = torch.Tensor(list(x.shape[-2:]))
        ksz = self.net.classifier.filter_size
        self.kernel_size2 = torch.Tensor([ksz, ksz] if isinstance(ksz, (int, float)) else ksz)
        self.output_sz2 = self.feature_sz2 + (self.kernel_size2 + 1) % 2

        # Construct output window
        self.output_window2 = None
        if self.params.get('window_output', False):
            if self.params.get('use_clipped_window', False):
                self.output_window2 = dcf.hann2d_clipped(self.output_sz2.long(), (
                        self.output_sz2 * self.params.effective_search_area / self.params.search_area_scale).long(),
                                                         centered=True).to(self.params.device)
            else:
                self.output_window2 = dcf.hann2d(self.output_sz2.long(), centered=True).to(self.params.device)
            self.output_window2 = self.output_window2.squeeze(0)

        # Get target boxes for the different augmentations
        target_boxes = self.init_target_boxes2()
        ...
        # Get target filter by running the discriminative model prediction module
        with torch.no_grad():
            self.target_filter2, _, losses = self.net.classifier.get_filter(x, target_boxes, num_iter=num_iter,
                                                                            compute_losses=plot_loss)

        # Init memory
        if self.params.get('update_classifier', True):
            self.init_memory2(TensorList([x]))

        if plot_loss:
            if isinstance(losses, dict):
                losses = losses['train']
            self.losses2 = torch.cat(losses)
            if self.visdom is not None:
                self.visdom.register((self.losses2, torch.arange(self.losses2.numel())), 'lineplot', 3,
                                     'Training Loss' + self.id_str2)
            elif self.params.debug >= 3:
                plot_graph(self.losses2, 10, title='Training Loss' + self.id_str2)
    ...
    def update_classifier2(self, train_x, target_box, learning_rate=None, scores=None):
        # Set flags and learning rate
        hard_negative_flag = learning_rate is not None
        if learning_rate is None:
            learning_rate = self.params.learning_rate
        ...
        # Update the tracker memory
        if hard_negative_flag or self.frame_num2 % self.params.get('train_sample_interval', 1) == 0:
            self.update_memory2(TensorList([train_x]), target_box, learning_rate)
        ...
        if hard_negative_flag:
            ...
            num_iter = self.params.get('net_opt_low_iter', None)
        elif (self.frame_num2 - 1) % self.params.train_skipping == 0:
            num_iter = self.params.get('net_opt_update_iter', None)
        ...
        if num_iter > 0:
            # Get inputs for the DiMP filter optimizer module
            samples = self.training_samples2[0][:self.num_stored_samples2[0], ...]
            target_boxes = self.target_boxes2[:self.num_stored_samples2[0], :].clone()
            sample_weights = self.sample_weights2[0][:self.num_stored_samples2[0]]

            # Run the filter optimizer module
            with torch.no_grad():
                self.target_filter2, _, losses = self.net.classifier.filter_optimizer(self.target_filter2,
                                                                                      num_iter=num_iter, feat=samples,
                                                                                      bb=target_boxes,
                                                                                      sample_weight=sample_weights,
                                                                                      compute_losses=plot_loss)
            ...
            if plot_loss:
                if isinstance(losses, dict):
                    losses = losses['train']
                self.losses2 = torch.cat((self.losses2, torch.cat(losses)))
                if self.visdom is not None:
                    self.visdom.register((self.losses2, torch.arange(self.losses2.numel())), 'lineplot', 3,
                                         'Training Loss' + self.id_str2)
                elif self.params.debug >= 3:
    ...
    def refine_target_box2(self, backbone_feat, sample_pos, sample_scale, scale_ind, update_scale=True):
        """Run the ATOM IoUNet to refine the target bounding box."""

        if hasattr(self.net.bb_regressor, 'predict_bb'):
            return self.direct_box_regression2(backbone_feat, sample_pos, sample_scale, scale_ind, update_scale)

        # Initial box for refinement
        init_box = self.get_iounet_box2(self.pos2, self.target_sz2, sample_pos, sample_scale)
        ...
        # Optimize the boxes
        output_boxes, output_iou = self.optimize_boxes2(iou_features, init_boxes)
        ...
        # Get new position and size
        new_pos = predicted_box[:2] + predicted_box[2:] / 2
        new_pos = (new_pos.flip((0,)) - (self.img_sample_sz2 - 1) / 2) * sample_scale + sample_pos
        new_target_sz = predicted_box[2:].flip((0,)) * sample_scale
        new_scale = torch.sqrt(new_target_sz.prod() / self.base_target_sz2.prod())

        self.pos_iounet2 = new_pos.clone()

        if self.params.get('use_iounet_pos_for_learning', True):
            self.pos2 = new_pos.clone()

        self.target_sz2 = new_target_sz

        if update_scale:
            self.target_scale2 = new_scale
    ...
    def optimize_boxes2(self, iou_features, init_boxes):
        box_refinement_space = self.params.get('box_refinement_space', 'default')
        if box_refinement_space == 'default':
            return self.optimize_boxes_default2(iou_features, init_boxes)
        if box_refinement_space == 'relative':
            return self.optimize_boxes_relative2(iou_features, init_boxes)
        raise ValueError('Unknown box_refinement_space {}'.format(box_refinement_space))
    ...
    def optimize_boxes_default2(self, iou_features, init_boxes):
        """Optimize iounet boxes with the default parametrization"""
        output_boxes = init_boxes.view(1, -1, 4).to(self.params.device)
        step_length = self.params.box_refinement_step_length
        if isinstance(step_length, (tuple, list)):
        ...
        for i_ in range(self.params.box_refinement_iter):
            ...
            outputs = self.net.bb_regressor.predict_iou(self.iou_modulation2, iou_features, bb_init)
    ...
    def optimize_boxes_relative2(self, iou_features, init_boxes):
        """Optimize iounet boxes with the relative parametrization ised in PrDiMP"""
        output_boxes = init_boxes.view(1, -1, 4).to(self.params.device)
        step_length = self.params.box_refinement_step_length
        if isinstance(step_length, (tuple, list)):
    ...
    def direct_box_regression2(self, backbone_feat, sample_pos, sample_scale, scale_ind, update_scale=True):
        """Implementation of direct bounding box regression."""
        ...
        # Optimize the boxes
        output_boxes = self.net.bb_regressor.predict_bb(self.iou_modulation2, iou_features, init_boxes).view(-1,
                                                                                                             4).cpu()
        ...
        new_pos = (new_pos.flip((0,)) - (self.img_sample_sz2 - 1) / 2) * sample_scale + sample_pos
        new_target_sz = predicted_box[2:].flip((0,)) * sample_scale
        new_scale_bbr = torch.sqrt(new_target_sz.prod() / self.base_target_sz2.prod())
        new_scale = new_scale_bbr
